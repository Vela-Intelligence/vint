...
function positions. Row identity holds within ONE `each` array: an item that
...
`Match(...)` calls. Each branch invocation must build FRESH DOM — a node a
...
`user()`) or edited through `mutate(prev => …)` — pick one.
