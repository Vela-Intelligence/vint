Build a live-updates widget with exact subscription discipline.
...
- A button whose text is `toggle`. The widget starts OFF.
...
- While OFF, an element whose text is `off` is shown, and there must be NO
  active subscription.
- While ON, the widget is subscribed to exactly the chosen channel and shows
