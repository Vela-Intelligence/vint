  ...
  mount(container, () => {
    ...
    const undo = () => {
      ...
      if (action.kind === "add") setCounters((prev) => prev.filter((c) => c.id !== action.id))
        ...
        setCounters((prev) =>
          prev.map((c) => (c.id === action.id ? { ...c, value: c.value - 1 } : c)),
        )
    }
    return div(
      ...
      ul(
        For({
          each: counters,
          key: (c) => c.id,
          // value-passing For: the row re-renders when its item changes
          children: (item) => li(String(item.value), button({ onclick: () => inc(item.id) }, "+")),
        }),
      ),
    )
  })
